...
nodebug:
	make "CFLAGS=-DSTATIC=static -DNODEBUG -O2" ek

#Minimum size and features.
min:
	make "CFLAGS=-DSTATIC=static -DMINSIZE -DOBUFLEN=256 -DFN_MAX=16 -O2" ek

#Ditto but Receive-Only:
minro:
	make "CFLAGS=-DSTATIC=static -DMINSIZE -DOBUFLEN=256 -DFN_MAX=16 -DRECVONLY -O2" ek

#Minimum size, receive-only, but with debugging:
minrod:
	make "CFLAGS=-DSTATIC=static -DMINSIZE -DOBUFLEN=256 -DFN_MAX=16 -DRECVONLY -DDEBUG -O2" ek
